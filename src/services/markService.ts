...
class MarkService {
	...
	async SaveNewCurrentLesson(newCurrentLessonDto: CurrentLessonDto) {
		...
		const arrayOneGroupStudents = await userService.GetStudentsByGroupId({
			groupId: newCurrentLessonDto.group._id,
		});
	...
	async SaveNewArrayCurrentLessons(arrayNewCurrentLessonDto: CurrentLessonDto[]) {
		const arrayOneGroupStudents = await userService.GetStudentsByGroupId({
			groupId: arrayNewCurrentLessonDto[0].group._id,
		});
	...
	private async saveEveryStudent(
		arrayOneGroupStudents: Array<HydratedDocument<IUser>>,
		newCurrentLessonDto: CurrentLessonDto,
		newCurrentLesson: ICurrentLesson
	) {
		for (const oneStudent of arrayOneGroupStudents) {
			const markOneStudent = await markRepository.GetAdditionalDataMarksOfOneStudent(
				oneStudent._id,
				newCurrentLessonDto.name._id
			);
