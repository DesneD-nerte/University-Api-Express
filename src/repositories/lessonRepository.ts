...
class LessonRepository {
	...
	async SaveNewCurrentLesson(newCurrentLessons: HydratedDocument<ICurrentLesson>) {
		const addedCurrentLesson = await (
			await newCurrentLessons.save()
		).populate(["name", "classroom", "group", "teachers"]);
	...
	async UpdateCurrentLesson(newCurrentLesson: HydratedDocument<ICurrentLesson>) {
		const updatedCurrentLesson = await CurrentLesson.findOneAndUpdate(
			{ _id: newCurrentLesson._id },
			newCurrentLesson
		);
