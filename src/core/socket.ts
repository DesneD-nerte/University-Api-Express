...
export default (http: http.Server) => {
	...
	io.on("connection", (socket: Socket) => {
		...
		socket.on("logged-in", (myId: string) => {
			...
			messagesService.GetLastMessages({ myId }).then((data) => {
		...
		socket.on("sendMessage", (data: any) => {
			...
			messagesService
				.AddMessage(body)
				.then((message) => {
					messagesService.GetLastMessages({ myId: body.myId }).then((lastMessages) => {
						io.to(connectedUsers[body.myId]).emit("updateLastMessages", lastMessages);
					});
					messagesService.GetLastMessages({ myId: body.id }).then((lastMessages) => {
						io.to(connectedUsers[body.id]).emit("updateLastMessages", lastMessages);
					});

					messagesService.GetMessages({ myId: body.myId, id: body.id, skip: 0 }).then((roomMessages) => {
		...
		socket.on("onEnterTheRoom", (data: IQueryMessage) => {
			messagesService.GetMessages(data).then((messages) => {
				if (messages) {
					socket.join(messages._id.toString());

					socket.emit("loadInitialRoomMessages", messages);
				} else {
					messagesService.AddRoom({ users: [data.myId, data.id] }).then((addedRoom) => {
